...
export default function Home() {
  ...
  useEffect(() => {
    ...
    document.body.style.overflow = selectedShot !== null || selectedFanArt !== null || selectedCpArt !== null || labOpen || easterEggOpen ? 'hidden' : '';
  ...
  return (
    <main className={soundOn ? 'is-listening' : undefined}>
      ...
      <section className="cp-section" id="cloud-aerith" style={{ '--cp-tone': cpMoments[activeCp].color } as React.CSSProperties}>
        ...
        <div className="cp-intimate-heading"><div><small>AFTER HOURS · FAN SPECIAL</small><h3>暧昧发生在，<br />再靠近一点之后。</h3></div></div>
      ...
      {easterEggOpen && (
        <div className="final-easter-egg" role="dialog" aria-modal="true" aria-labelledby="final-egg-title" onClick={closeEasterEgg}>
          ...
          <div className="egg-comets" aria-hidden="true">{Array.from({ length: 9 }).map((_, index) => <i key={index} style={{ left: `${6 + (index * 19) % 88}%`, top: `${5 + (index * 13) % 52}%`, animationDelay: `${index * -1.7}s`, animationDuration: `${9.5 + index % 4 * 1.2}s` }} />)}</div>
          <div className="egg-lightfall" aria-hidden="true">{Array.from({ length: 16 }).map((_, index) => <i key={index} style={{ left: `${2 + (index * 17) % 96}%`, animationDelay: `${index * -.85}s`, animationDuration: `${11 + index % 5 * 1.35}s` }} />)}</div>
          ...
          <div className="egg-particles egg-particles-back" aria-hidden="true">{Array.from({ length: 32 }).map((_, index) => <i key={index} style={{ left: `${(index * 37) % 100}%`, animationDelay: `${(index % 11) * -.95}s`, animationDuration: `${14 + (index % 6) * 1.25}s` }} />)}</div>
          <div className="egg-particles egg-particles-front" aria-hidden="true">{Array.from({ length: 22 }).map((_, index) => <i key={index} style={{ left: `${(index * 43 + 9) % 100}%`, animationDelay: `${(index % 8) * -1.2}s`, animationDuration: `${11.5 + (index % 5) * 1.1}s` }} />)}</div>
